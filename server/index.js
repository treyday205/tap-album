...
const PWA_MANIFEST_VERSION = String(process.env.PWA_MANIFEST_VERSION || '2026.02.17.3').trim() || '2026.02.17.3';
...
const isAudioAssetKey = (value) => {
  const key = String(value || '').trim().toLowerCase();
  if (!key) return false;
  if (key.includes('/tracks/')) return true;
...
app.post('/api/uploads/presign', async (req, res) => {
  ...
  const allowedExtensions = normalizedKind === 'track-audio' ? TRACK_AUDIO_EXTENSIONS : IMAGE_EXTENSIONS;
  const extension =
  ...
  if (!extension) {
    return res.status(400).json({
      message: normalizedKind === 'track-audio'
        ? 'Unsupported audio type. Only MP3 uploads are supported.'
...
app.post('/api/assets/sign', async (req, res) => {
  ...
  try {
    ...
    for (const ref of assetRefs) {
      ...
      const isAudioKey = isAudioAssetKey(key) && key.includes(`/${safeProjectId}/`);
...
app.get('/api/projects/:projectId/security-stats', async (req, res) => {
  const safeProjectId = safeSegment(req.params.projectId);
  if (!safeProjectId) {
    return res.status(400).json({ message: 'projectId is required.' });
  }
  ...
  try {
    const result = await query(
      'SELECT project_id, pin_unlock_count, pin_active_count, data, updated_at FROM projects WHERE project_id = $1 LIMIT 1',
      [safeProjectId]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ message: 'Project not found.' });
    ...
    const projectCapacityStats = buildProjectCapacityStats(row);
    res.set('Cache-Control', 'no-store');
    return res.json({
      success: true,
      projectId: row.project_id,
      pinUnlockUsed: projectCapacityStats.unlocksUsed,
      pinUnlockRemaining: projectCapacityStats.unlocksRemaining,
      pinUnlockLimit: projectCapacityStats.unlocksLimit,
      pinActiveUsed: projectCapacityStats.activePinsUsed,
      pinActiveRemaining: projectCapacityStats.activePinsRemaining,
      pinActiveLimit: projectCapacityStats.activePinsLimit,
      updatedAt: row.updated_at
    });
  } catch (err) {
  ...
  try {
    ...
    for (let index = 0; index < trackRows.length; index += 1) {
      ...
      const trackNo = Number.isFinite(trackNoValue)
        ...
        : index + 1;
      const sortOrderValue = Number(track.sortOrder ?? trackNo);
...
app.get('/api/pwa/manifest', async (req, res) => {
  ...
  const manifestVersion = requestVersion || normalizeManifestVersion(PWA_MANIFEST_VERSION) || '2026.02.17.3';
