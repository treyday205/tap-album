...
app.post('/api/projects', async (req, res) => {
  ...
  try {
    ...
    if (!forceNew) {
      const reusableDraft = draftResult.rows.find((row) => !isAutoGeneratedGhostProjectRow(row));
      if (reusableDraft) {
        const project = await buildProjectPayload(reusableDraft, { includeSignedCover: true });
        res.set('Cache-Control', 'no-store');
        return res.json({
          success: true,
          created: false,
...
app.delete('/api/projects/:projectId', async (req, res) => {
  ...
  try {
    ...
    await query('DELETE FROM projects WHERE project_id = $1', [safeProjectId]);
    res.set('Cache-Control', 'no-store');
    return res.json({
      success: true,
      deleted: true,
  ...
  try {
    ...
    const updateResult = await client.query(
      `UPDATE projects
       ...
       WHERE project_id = $1
       RETURNING *`,
      [payload.projectId, normalizedSlug, title, artistName, coverImageUrl, published, payload]
    );
