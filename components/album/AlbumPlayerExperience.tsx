...
type AlbumPlayerExperienceProps = {
  album: AlbumPayload;
...
}> = ({ open, title, cards, fallbackUrl, onClose, position }) => {
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          className={`${position} inset-0 z-[70]`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
...
const AlbumPlayerExperience: React.FC<AlbumPlayerExperienceProps> = ({
  album,
  ...
  const handleShare = async () => {
    setShareError(null);
    setShareStatus('idle');
    const targetUrl = String(shareUrl || (typeof window !== 'undefined' ? window.location.href : '')).trim();
    if (!targetUrl) {
    ...
    try {
      ...
      console.warn('[ALBUM_PLAYER] share failed', error);
      setShareError('Share unavailable on this device.');
    }
  };
  ...
  const shellClass = isEmbeddedPreview
    ? 'relative h-full overflow-hidden overflow-x-hidden bg-slate-950 text-slate-50'
    : 'tap-full-height relative overflow-x-hidden bg-slate-950 text-slate-50';
  const scrollContainerClass = isEmbeddedPreview
    ? 'relative h-full overflow-y-auto tap-native-scroll'
    : 'relative';
  const overlayPosition: OverlayPosition = isEmbeddedPreview ? 'absolute' : 'fixed';
  const floatingClass = isEmbeddedPreview ? 'absolute' : 'fixed';
  ...
  return (
    <div className={shellClass}>
      ...
      <AnimatePresence>
        {isPlaying && currentTrack && activeTab !== 'album' && (
          <motion.button
            ...
            className={`${floatingClass} inset-x-0 bottom-24 z-50 mx-auto flex w-[calc(100%-1.5rem)] max-w-md items-center gap-3 rounded-2xl border border-slate-700 bg-slate-900/95 px-3 py-2 text-left shadow-2xl backdrop-blur`}
          >
      ...
      <div className={`${floatingClass} bottom-0 inset-x-0 z-40 border-t border-slate-800 bg-slate-950/95 backdrop-blur`}>
...
export default AlbumPlayerExperience;
